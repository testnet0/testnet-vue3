...
<script lang="ts" name="cn.iotaa-assetIp" setup>
  ...
  async function handleBatchRunChain(id, chainName) {
    batchRunChain(
      { queryObject: queryObject, queryParam: queryParam, chainId: id, chainName: chainName, data: rowSelection.selectedRows, assetType: 'ip' },
      handleSuccess
    );
  }
  ...
  async function batchHandleDelete() {
    if (selectedRowKeys.value && selectedRowKeys.value.length > 0) {
      ...
      batchDeleteBySearch({ queryObject, queryParam, assetType: 'ip' }, handleSuccess);
    }
  }
  /**
   * 成功回调
