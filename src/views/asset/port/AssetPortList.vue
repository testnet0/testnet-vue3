...
<script lang="ts" name="cn.iotaa-assetPort" setup>
  ...
  async function batchHandleDelete() {
    if (selectedRowKeys.value && selectedRowKeys.value.length > 0) {
      ...
      batchDeleteBySearch({ queryObject, queryParam, assetType: 'port' }, handleSuccess);
    }
  }
  ...
  async function handleBatchRunChain(id, chainName) {
    batchRunChain(
      { queryObject: queryObject, queryParam: queryParam, chainId: id, chainName: chainName, data: rowSelection.selectedRows, assetType: 'port' },
      handleSuccess
    );
  }
