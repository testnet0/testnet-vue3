<template>
  <div>
    ...
    <!-- 表单区域 -->
    <AssetPortModal @register="registerModal" @success="handleSuccess" />
    <SelectAssetLabelModel @register="registerLabelModal" @success="handleSuccess" />
  </div>
</template>

<script lang="ts" name="cn.iotaa-assetPort" setup>
  import { ref, onMounted, unref, reactive } from 'vue';
  import { BasicTable, useTable, TableAction } from '/@/components/Table';
  import { useModal } from '/@/components/Modal';
  import { useListPage } from '/@/hooks/system/useListPage';
  import AssetPortModal from './components/AssetPortModal.vue';
  import { columns, searchFormSchema, superQuerySchema } from './AssetPort.data';
  import { list, deleteOne, batchDelete, getImportUrl, getExportUrl } from './AssetPort.api';
  ...
  async function batchHandleDelete() {
    if (selectedRowKeys.value && selectedRowKeys.value.length > 0) {
      ...
      batchDeleteBySearch({ queryObject: { ...queryObject, ...queryParam }, assetType: 'port' }, handleSuccess);
    }
  }
  ...
  async function handleBatchRunChain(id, chainName) {
    batchRunChain(
      { queryObject: { ...queryObject, ...queryParam }, chainId: id, chainName: chainName, data: rowSelection.selectedRows, assetType: 'port' },
      handleSuccess
    );
  }
  ...
  async function handleRunChain(id, chainName, record) {
    await runChain({ chainId: id, chainName: chainName, data: [record], assetType: 'port' });
  }

  const fetchDropdownItems = async () => {
    try {
      const params = { assetType: 'port' };
